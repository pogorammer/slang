...
func TestErrorHandling(t *testing.T) {
	tests := []struct {
		...
		{`{[1, 2]: "Slang"}`, "unusable as hash key: Array"},
		{`{"name": "Slang"}[fucks(x) { x }]`, "unusable as hash key: Function"},
	}
...
func TestFuckStatements(t *testing.T) {
	tests := []struct {
		input    string
		expected int64
	}{
		{"fuck a = 5; a;", 5},
		{"fuck a = 5 * 5; a;", 25},
		{"fuck a = 5; fuck b = a; b;", 5},
		{"fuck a = 5; fuck b = a; fuck c = a + b + 5; c;", 15},
	}
...
func TestFunctionObject(t *testing.T) {
	...
	fucks, ok := evaluated.(*object.Function)
	if !ok {
		t.Fatalf("object is not *object.Function. got=%#v", evaluated)
	}

	if len(fucks.Parameters) != 1 {
		t.Fatalf("function has wrong parameters. got=%+v", fucks.Parameters)
	}

	if fucks.Parameters[0].String() != "x" {
		t.Fatalf("parameter is not 'x'. got=%q", fucks.Parameters[0])
	}

	expectedBody := "(x + 2)"
	if body := fucks.Body.String(); body != expectedBody {
...
func TestFunctionApplication(t *testing.T) {
	tests := []struct {
		...
		{"fuck identity = fucks(x) { x; }; identity(5);", 5},
		{"fuck identity = fucks(x) { return x; }; identity(5);", 5},
		{"fuck double = fucks(x) { x * 2; }; double(5);", 10},
		{"fuck add = fucks(x, y) { x + y; }; add(5, 5);", 10},
		{"fuck add = fucks(x, y) { x + y; }; add(5 + 5, add(5, 5));", 20},
		{"fucks(x) { x; }(5);", 5},
	}
...
func TestArrayIndexExpressions(t *testing.T) {
	tests := []struct {
		...
		{"fuck i = 0; [1][i]", 1},
		{"[1, 2, 3][1 + 1]", 3},
		{"fuck arr = [1, 2, 3]; arr[2];", 3},
		{"fuck arr = [1, 2, 3]; arr[0] + arr[1] + arr[2];", 6},
		{"fuck arr = [1, 2, 3]; fuck i = arr[0]; arr[i]", 2},
		{"[1, 2, 3][3]", nil},
		{"[1, 2, 3][-1]", nil},
	}
