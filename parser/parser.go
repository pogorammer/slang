...
type (
	prefixParseFucks func() ast.Expression
	infixParseFucks  func(ast.Expression) ast.Expression
)

// Parser is a parser of Slang programming language.
type Parser struct {
	l      lexer.Lexer
	errors []string
	...
	prefixParseFuckss map[token.Type]prefixParseFucks
	infixParseFuckss  map[token.Type]infixParseFucks
}
...
func New(l lexer.Lexer) *Parser {
	...
	p.prefixParseFuckss = map[token.Type]prefixParseFucks{
	...
	p.infixParseFuckss = map[token.Type]infixParseFucks{
...
func (p *Parser) parseFuckStatement() *ast.FuckStatement {
	stmt := &ast.FuckStatement{Token: p.curToken}
...
func (p *Parser) parseExpression(precedence int) ast.Expression {
	prefix := p.prefixParseFuckss[p.curToken.Type]
	if prefix == nil {
	...
	for !p.curTokenIs(token.SEMICOLON) && precedence < p.peekPrecedence() {
		infix := p.infixParseFuckss[p.peekToken.Type]
		if infix == nil {
			return leftExp
		}
